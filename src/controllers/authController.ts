...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    return successResponse(res,  "User registered successfully, check email to verify your account", {userId: user._id, email: user.email});
  } catch (err: any) {
...
export const requestEmailVerification = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = req.user;
    ...
    return successResponse(res, "Verification OTP sent to email", {userId: user._id, email: user.email});
  } catch (err: any) {
    console.error(err);
    return errorResponse(res, err.message, 500);
  }
